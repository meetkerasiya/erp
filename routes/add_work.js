...
router.post("/",[auth],async (req,res)=>{
    ...
    const {err} =validation(req.username,req.body.details,req.body.client,req.body.cost,req.body.faculty_involved);
    if(err)
    {
        return res.set(401).send(err.details[0].message);
    }
    const check=await Work.findOne({client: req.body.client,username: req.username});
    if(check != null)
    {/*
        ...
        res.send("This Work already exists")
    }
    else
    {
        let work=new Work({
